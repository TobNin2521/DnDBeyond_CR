...
function mainOverview() {
    let partyStats = document.querySelectorAll(".qa-party_composition-summary");
    let partyStatsString = partyStats[0].innerText.replaceAll("players of level ", "").replaceAll(" (avg)", "").split(' ');
    let partyLevel = Number(partyStatsString[1]);
    let charCount = Number(partyStatsString[0]);
    ...
    let partyCR = getXpBudget();
    let totalXP = Number(document.querySelectorAll(".qa-total_xp-value")[0].innerText.replaceAll(" XP", ""));
    
    let diff = "Trivial";
    if(totalXP >= partyCR.low) diff = "Low";
    if(totalXP >= partyCR.moderate) diff = "Moderate";
    if(totalXP >= partyCR.high) diff = "High";
    document.querySelectorAll(".qa-difficulty-value.difficulty-text.difficulty-text--hard")[0].innerText = diff;
    ...
    document.querySelectorAll(".difficulty-bar.encounter-details-difficulty-summary__difficulty-bar")[0].style['display'] = 'none';
    document.querySelector(".encounter-details-difficulty-summary__stats").after(buldDiffBar(partyCR, totalXP));

};

function wait() {
    if(window.location.href.indexOf("encounter-builder") >= 0) {
        let partyStats = document.querySelectorAll(".party-stats__stat-value");
    ...
    if(window.location.href.indexOf("/encounters/") >= 0) {
        let partyStats = document.querySelectorAll(".qa-party_composition-summary");
        let partyXP = document.querySelectorAll(".qa-total_xp-value");
        if(partyStats !== null && partyStats.length < 1 && partyXP !== null && partyXP.length < 1) {
            setTimeout(wait, 50);
        }
        else mainOverview();
    }
